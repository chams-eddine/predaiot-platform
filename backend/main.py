...
latest_live_result = {"edv_optimal_total": 0, "edv_actual_total": 0, "dq_score": 0, "total_gap_usd": 0, "decision_log": []}
shared_audits = {} # ذاكرة مؤقتة لروابط المشاركة

# ==========================================
# 4. محرك التحسين والمحاسبة المركزي (لمنع التكرار)
# ==========================================
def run_optimizer(asset: AssetSpecs, prices: List[float]) -> dict:
    ...
    for t in hours:
        ...
        if t == 0: prob += soc[t] == asset.soc_init + (p_ch[t] * asset.eta_ch - p_dis[t] / asset.eta_dis) / asset.e_max
        else: prob += soc[t] == soc[t-1] + (p_ch[t] * asset.eta_ch - p_dis[t] / asset.eta_dis) / asset.e_max
        if t == max(hours): prob += soc[t] == asset.soc_init
    prob.solve()
...
def process_calculation(asset: AssetSpecs, time_series_list: list, save_to_db: bool = True):
    ...
    total_edv_opt, total_edv_act, decision_log = 0, 0, []
    ...
    try:
        for i, ts in enumerate(time_series_list):
            ...
            if db:
                db.add(DecisionAuditLog(asset_id="BESS_01", timestamp=datetime.utcnow(), market_price=ts["price"], optimal_action=opt_dis, actual_action=act_dis, economic_gap=gap_step))
            decision_log.append(DecisionRecord(hour=ts.get("hour"), price=ts["price"], optimal_action=round(opt_dis,2), actual_action=round(act_dis,2), edv_optimal_step=round(edv_opt_step,2), edv_actual_step=round(edv_act_step,2), gap_step=round(gap_step,2)))
        if db: db.commit()
    finally:
        if db: db.close()
        
    dq_score = (total_edv_act / total_edv_opt) if total_edv_opt > 0 else 0
    latest_live_result = AuditResponse(edv_optimal_total=round(total_edv_opt,2), edv_actual_total=round(total_edv_act,2), dq_score=round(dq_score,4), total_gap_usd=round(total_edv_opt - total_edv_act, 2), decision_log=decision_log)
...
async def audit_from_file(file: UploadFile = File(...)):
    try:
        ...
        if file.filename.endswith(('.xlsx', '.xls')): df = pd.read_excel(BytesIO(contents))
        else: df = pd.read_csv(BytesIO(contents.decode('utf-8')))
        
        df.columns = df.columns.str.lower()
        required = {'price', 'actual_discharge'}
        if not required.issubset(df.columns):
            return JSONResponse(status_code=400, content={"detail": "Missing columns. Ensure CSV/Excel has 'price' and 'actual_discharge' columns."})
            
        time_series = df[['price', 'actual_discharge']].fillna(0).to_dict('records')
        asset = AssetSpecs() # استخدام الإعدادات الافتراضية للملفات المرفوعة
