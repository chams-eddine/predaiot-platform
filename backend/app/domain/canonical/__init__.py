...
and `to_wire()` flattens to the frozen engine, plus the evidence-based
FacilityProfile (S3) that resolves into the Composer input. Depends only on
schemas + knowledge; never the engine (Law 1: Immutable Engine).
"""
from app.domain.canonical.model import (  # noqa: F401
    CapabilityRef, Constraint, Intent, Equipment, Process, Facility,
    ResolvedEquipment, ResolvedProfile,
)
from app.domain.canonical.profile import (  # noqa: F401
