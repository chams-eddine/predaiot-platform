...
function App() {
  ...
  // حالة جديدة لبيانات الـ 30 يوماً
  ...
  useEffect(() => {
    const fetchLiveSCADA = async () => {
      try {
        ...
        if (response.data && response.data.dq_score > 0) setData(response.data);
      } catch (error) {}
    };
    fetchLiveSCADA();
  ...
  const fetchAuditData = async () => {
    ...
    try {
      const time_series = [];
      let actual_soc = 0.2; const e_max = 100;
      for (let i = 0; i < 288; i++) {
        ...
        if (price < 20 && actual_soc < 0.9) actual_soc += (40 * 0.95 / e_max);
      ...
      const payload = { asset: { p_max: 50, e_max: 100, soc_init: 0.2, eta_ch: 0.95, eta_dis: 0.95, deg_cost: 5.0 }, time_series };
  ...
  // الدالة السحرية: جلب بيانات الـ 30 يوماً من السحابة
  ...
  const worstDecisions = [...logData].sort((a, b) => (b.gap_step || 0) - (a.gap_step || 0)).slice(0, 3);
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '15px' }}>
        <h1 style={{ margin: 0, fontSize: '24px', fontWeight: '300', letterSpacing: '2px' }}>PREDAIOT <span style={{ color: '#00E5FF', ...styles.neonText }}>INTELLIGENCE</span></h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '20px', marginBottom: '30px' }}>
        <div style={{ ...styles.card, marginTop: '0' }}><div style={{ color: '#888', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '1px' }}>Optimal Potential</div><div style={{ fontSize: '32px', fontWeight: '700', color: '#FFD700', marginTop: '10px' }}>${data.edv_optimal_total.toLocaleString()}</div></div>
        <div style={{ ...styles.card, marginTop: '0' }}><div style={{ color: '#888', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '1px' }}>Actual Captured</div><div style={{ fontSize: '32px', fontWeight: '700', color: '#00E5FF', marginTop: '10px' }}>${data.edv_actual_total.toLocaleString()}</div></div>
        <div style={{ ...styles.card, borderColor: 'rgba(255, 0, 85, 0.3)', marginTop: '0' }}><div style={{ color: '#FF0055', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '1px' }}>Value Destroyed (Gap)</div><div style={{ fontSize: '32px', fontWeight: '700', color: '#FF0055', marginTop: '10px', ...styles.redGlow }}>-${data.total_gap_usd.toLocaleString()}</div></div>
        <div style={{ ...styles.card, marginTop: '0' }}><div style={{ color: '#888', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '1px' }}>Decision Quality (DQ)</div><div style={{ fontSize: '42px', fontWeight: '800', color: data.dq_score >= 0.8 ? '#00FF00' : data.dq_score >= 0.6 ? '#FFAA00' : '#FF0055', marginTop: '10px' }}>{(data.dq_score * 100).toFixed(1)}<span style={{ fontSize: '20px' }}>%</span></div></div>
      </div>

      <button onClick={fetchAuditData} disabled={loading} style={{ padding: '12px 24px', backgroundColor: 'transparent', color: '#00E5FF', border: '1px solid #00E5FF', borderRadius: '8px', cursor: loading ? 'not-allowed' : 'pointer', marginBottom: '30px', fontSize: '14px', letterSpacing: '1px', fontWeight: '600' }}>
        {loading ? 'OPTIMIZING 288 NODES...' : 'RUN 24-HR FORENSIC AUDIT'}
      </button>

      {/* قسم التدقيق اللحظي (الرسوم البيانية والأخطاء) */}
        ...
        <div style={styles.card}>
          ...
          <ResponsiveContainer width="100%" height={350}>
            <ComposedChart data={logData} margin={{ top: 10, right: 0, left: 0, bottom: 0 }}>
              <defs><linearGradient id="gapFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#FF0055" stopOpacity={0.4}/><stop offset="100%" stopColor="#FF0055" stopOpacity={0.05}/></linearGradient></defs>
              <CartesianGrid strokeDasharray="3 3" stroke="#222" />
              <XAxis dataKey="hour" stroke="#555" tickFormatter={formatTime} interval={23} />
              <YAxis stroke="#555" tickFormatter={(v) => `$${v}`} />
              <Tooltip contentStyle={{ backgroundColor: '#111', border: '1px solid #333', borderRadius: '8px' }} labelFormatter={formatTime} formatter={(value, name) => { const colors = { 'Optimal Path': '#FFD700', 'Actual Path': '#00E5FF', 'Economic Gap': '#FF0055' }; return [`$${value}`, <span key={name} style={{ color: colors[name] }}>{name}</span>]; }} />
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
            {worstDecisions.length > 0 && worstDecisions[0].gap_step > 0 ? worstDecisions.map((dec, index) => (
              <div key={`failure-${index}`} style={{ backgroundColor: 'rgba(255, 0, 85, 0.05)', border: '1px solid rgba(255, 0, 85, 0.2)', borderRadius: '8px', padding: '15px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}><span style={{ color: '#888', fontSize: '13px' }}>TIME: {formatTime(dec.hour)}</span><span style={{ color: '#FF0055', fontWeight: 'bold' }}>-${dec.gap_step.toLocaleString()}</span></div>
                <div style={{ fontSize: '12px', color: '#AAA', lineHeight: '1.5' }}><span style={{ color: '#FFD700' }}>Should have:</span> Discharge {dec.optimal_action} MW @ ${dec.price}<br/><span style={{ color: '#00E5FF' }}>Actually did:</span> Discharge {dec.actual_action} MW @ ${dec.price}</div>
              </div>
      ...
      {/* ========================================== */}
      {/* القسم السحري الجديد: النزيف المالي لـ 30 يوماً */}
      {/* ========================================== */}
      <div style={styles.card}>
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={historyData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
            ...
            <Tooltip 
              ...
              formatter={(value, name) => [`$${value} Lost`, name]}
            />
        ...
        {historyData.length === 0 && !historyLoading && (
          <div style={{ textAlign: 'center', color: '#444', marginTop: '20px', fontSize: '13px' }}>
            Waiting for historical data... (Ensure SCADA bridge is running locally to populate the cloud DB).
          </div>
        )}
      </div>
